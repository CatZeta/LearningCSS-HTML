  ...
  <div class="action-bar" v-if="stocks">
    <div v-for="stock in stocks" :key="stock.id" class="company">
      ...
      <span :id="getPercentageClass(stock.percentage)">
                <span id="arrows" class="material-symbols-outlined">
                  {{ getPercentageArrowIcon(stock.percentage) }}
                </span>
...
function getPercentageArrowIcon(percentage) {
  return percentage > 0 ? 'arrow_upward' : 'arrow_downward';
}
